...
function parse(ast) {
  ...
  var walker = new UglifyJS.TreeWalker(function(node, descend) {
    ...
    if (node instanceof UglifyJS.AST_Call && node.expression.name === 'define') {
      var define = getDefine(node, walker);
      if (define) {
        meta.push(define);
      }
      return true;
...
function getAstBySymbolDef(ast, parentWalker) {
  if(ast instanceof UglifyJS.AST_SymbolRef && parentWalker && parentWalker.stack.length > 1) {
    for(var i = parentWalker.stack.length - 2; i >= 0; i--) {
      factoryCall = parentWalker.stack[i];
      if(factoryCall instanceof UglifyJS.AST_Call) {
        if(factoryCall.args && factoryCall.args.length
          && factoryCall.expression.argnames
          && factoryCall.expression.argnames.length) {
          for(var j = 0; j < factoryCall.expression.argnames.length; j++) {
            if(factoryCall.expression.argnames[j].name === ast.name) {
              return factoryCall.args[j];
            }
          }
        }
      }
...
function getDefine(node, parentWalker) {
  ...
  if (node instanceof UglifyJS.AST_Call && node.expression.name === 'define') {
    ...
    if (node.args.length === 1) {
      ...
      } else if (factory instanceof UglifyJS.AST_SymbolRef) {
        dependencies = getRequires(getAstBySymbolDef(factory, parentWalker));
      }
